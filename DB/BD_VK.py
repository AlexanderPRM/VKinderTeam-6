    ...
    with conn.cursor() as cur:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS Favorites(
                ...
                id_client_f INTEGER NOT NULL REFERENCES Client(id_client)
                    ON DELETE CASCADE,
                name_f VARCHAR(40),
...
def insert_client(
    conn, name: str, surname: str,
        city: str, age: int, id_vk_c: int, gender: str):
    """
    Функция заполнения таблицы Пользователь.
    Функция принимает на вход данные о пользователе, исходя из его профиля ВК
    """
    with conn.cursor() as cur:
        cur.execute(f"""INSERT INTO Client(name, surname, city,
                                            age, id_vk_c, gender)
                        VALUES('{name}', '{surname}', '{city}', '{age}',
                                '{id_vk_c}', '{gender}')
                        RETURNING id_client;""")
...
def insert_favorites(
                conn, id_client_f: int, name_f: str, surname_f: str,
                city_f: str, age_f: int,
                id_vk_f: int, gender_f: str):
    """
    Функция заполнения таблицы Избранное
    Функция принимает на вход данные об "избранных",
    которых выберет пользователь, исходя из их профиля ВК
    """
    with conn.cursor() as cur:
        cur.execute(f"""INSERT INTO Favorites(id_client_f, name_f, surname_f,
                                            city_f, age_f, id_vk_f, gender_f)
                        VALUES('{id_client_f}','{name_f}', '{surname_f}',
                                '{city_f}', '{age_f}', '{id_vk_f}',
...
def insert_blocked(conn, id_client_b: int, id_vk_b: int):
    ...
    Функция принимает на вход данные об "заблокированных",
    которых выберет пользователь, исходя из их профиля ВК
    """
    with conn.cursor() as cur:
        cur.execute(f"""INSERT INTO Blocked(id_client_b, id_vk_b)
                        VALUES('{id_client_b}', '{id_vk_b}')
                        RETURNING id_blocked;""")
        conn.commit()
...
def search_count(conn, check: int):
    ...
    Функция принимает на вход id ВК пользователя,
    и возвращает id Пользователя в таблице,
...
def result_favorites(conn, check: int):
    ...
    with conn.cursor() as cur:
        cur.execute(f"""
                SELECT name_f, surname_f, city_f, age_f, id_vk_f, gender_f
                FROM Favorites
                JOIN Client ON  Client.id_client = id_client_f
                WHERE id_client_f =(SELECT id_client FROM Client
                                    WHERE id_vk_c= {check});
                    """)
        return cur.fetchall()
...
def result_blocked(conn, check: int):
    ...
    Функция принимает на вход id ВК пользователя,
     и возвращает список людей в ЧС,
    которые соответствуют этому пользователю
    """
    with conn.cursor() as cur:
        cur.execute(f"""
                ...
                WHERE id_client_b =(SELECT id_client FROM Client
                                    WHERE id_vk_c = {check});
                    """)
        result_list_b = []
...
def result_favorites_id(conn, check: int):
    ...
    Функция принимает на вход id ВК пользователя,
     и возвращает список id VK избранных,
    которые соответствуют этому пользователю.
    Нужна для отсекания повтора показа ранее избранных.
    """
    with conn.cursor() as cur:
        cur.execute(f"""
...
def result_favorites_blocked(conn, check):
    """
        Функция вывода ID VK,
        избранных и заблокированных для текущего пользователя.
        Функция принимает на вход id ВК пользователя,
        и возвращает список id VK избранных и заблокированных,
        которые соответствуют этому пользователю.
        Нужна для отсекания повтора показа ранее избранных и заблокированных.
        """
    with conn.cursor() as cur:
        cur.execute(f"""
                SELECT id_vk_f FROM Favorites
...
def result_favorites_first(conn):
    """
    Функция вывода избранных для пользователя,
    который воспользовался программой в первый раз
...
def result_blocked_first(conn):
    """
    Функция вывода чc для пользователя,
    который воспользовался программой в первый раз
...
def check_client(conn, check: int):
    ...
    Функция принимает на вход ID пользователя в ВК,
    и ищет соответствие ранее записанных id с введенным,
...
def delete_favorites(conn, check: int):
    """
        Функция изменения города при повторном заходе,
        и удаления записей из избранного с прежним городом.
        Функция принимает на вход id пользователя ВК и название нового города,
        производит замену названия города в таблице для пользователей,
        и удаляет записи в таблице "избранное",
        которые были с прежним названием города.
        """
    with conn.cursor() as cur:
        cur.execute(f"""
                    DELETE FROM Favorites WHERE city_f IN (SELECT city_f
                                                            FROM Favorites
                    JOIN Client ON  Client.id_client = id_client_f
                    WHERE  id_vk_c = {check});
                     """)
