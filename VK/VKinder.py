...
from DB.BD_VK import (check_client, delete_favorites, insert_blocked,
                      insert_client, insert_favorites, result_favorites,
                      search_count, update_user_city)
...
class VKinder:
    ...
    def send_photos(
                self, user_id,
                likes, owner_id: int,
                first_name, age, keyboard=None):
        """
            Метод, выполняющий отправку фото подходящего человека пользователю.
            Используется только в связке
            с другими функциями.
        """
        ...
        if len(id_photos) == 2:
            photos = f"photo{owner_id}_{id_photos[0]}," + \
                f"photo{owner_id}_{id_photos[1]}"
        elif len(id_photos) == 1:
            photos = f"photo{owner_id}_{id_photos[0]}"
        else:
            photos = f"photo{owner_id}_{id_photos[0]}," + \
                f"photo{owner_id}_{id_photos[1]}," + \
                f"photo{owner_id}_{id_photos[2]}"
    ...
    def wait_message(self, longpoll):
        """
        Данная повторяющаяся конструкция ждёт сообщение,
        которое пришло именно боту.
        """
    ...
    def bot_keyboard(self):
        ...
        rate.add_button(
            ...
            color=keyboard.VkKeyboardColor.SECONDARY
            )

        start_search = keyboard.VkKeyboard(one_time=True)
        start_search.add_button(
                label='Поиск',
                color=keyboard.VkKeyboardColor.POSITIVE)

        next_people_button = keyboard.VkKeyboard(one_time=True)
        next_people_button.add_button(label='Смотреть следующих.')
        next_people_button.add_button(
            label='Остановить поиск',
            color=keyboard.VkKeyboardColor.SECONDARY
            )
        return rate, start_search, next_people_button

    def photos(self, user_id, db_client_id,
               user, suitable, counter: int, user_city):
        """
        Метод по отправке данных подходящего человека.
        Используется в связке с функцией listen.
        В функции используется несколько рекурсий,
        без них бот перестаёт обрабатывать сообщения, каждая рекурсия функции,
        новый человек.
        """
        print('yes')
        rate, start_search, next_people_button = self.bot_keyboard()
        longpoll = VkLongPoll(self.public_vk)
        if not user.enumeration_ids(counter=counter,
                                    suitable=suitable) is None:
            try:
                owner_id, first_name, last_name, \
                    age, sex, city, counter = user.enumeration_ids(
                                                counter=counter,
                                                suitable=suitable
                                            )
            except ValueError:
                self.write_msg(
                    user_id,
                    'Вы просмотрели весь список подходящих.'
                    'Хотите увидеть следующий?',
            ...
            try:
                owner_id, first_name, last_name, \
                    age, city, sex, counter = user.enumeration_ids(
                                                    counter=counter + 1,
                                                    suitable=suitable)
            except ValueError:
                self.write_msg(user_id,
                               '''Вы просмотрели весь первый список подходящих.
                               Хотите увидеть следующий?''',
                               keyboard=next_people_button.get_keyboard())
        ...
        if request.strip() == 'Остановить поиск':
            ...
            if choice.strip().lower() in ['1', 'да']:
                ...
                for favorite in favorites:
                    text += f'{favorite[0]}, {favorite[3]} - '
                    f'https://vk.com/id{favorite[4]}\n'
                self.write_msg(user_id,
                               f'Люди выбранные вами.\n\n{text}')
            delete_favorites(self.conn, user_id)
            self.write_msg(user_id,
                           '''Для повторного поиска введите команду "Поиск"
                           или нажмите на кнопку.''',
                           keyboard=start_search.get_keyboard())
            self.listen()
        elif request.strip() == '❤':
            insert_favorites(
                self.conn, db_client_id, first_name,
                last_name, city, age, owner_id, sex
                )

            self.write_msg(
                user_id,
                f'[id{owner_id}|'
                'Для дальнейшего общения нажимай на меня &#9995;]')
            self.photos(
                user_id=user_id, db_client_id=db_client_id,
                user=user, suitable=suitable, counter=counter,
                user_city=user_city)
        elif request.strip() == '👎🏻':
            insert_blocked(self.conn, db_client_id, owner_id)
            self.photos(user_id=user_id, db_client_id=db_client_id,
                        user=user, suitable=suitable, counter=counter,
                        user_city=user_city)
        else:
    ...
    def search_peoples(self, user_id, longpoll, offset=0, user_city=None):
        ...
        выведен в отдельный метод для того чтобы была возможность
        использовать в рекурсии,
        и при остановке поиска или просмотра всех загруженных подходящих,
        не требовалось начинать с самого начала логики бота

        user_city - параметр которая указывается при рекурсии,
        suitable - список словарей, в котором находятся подходящие,
        далее делаем по нему цикл.
        """
        user_id = user_id
        user = VkUser(user_id, self.conn)
        f_name, l_name, age, city, relation, sex = user.get_user_info(user_id)
        # Проверка первый раз вызвана или рекурсивно.
        ...
        for client in suitable:
            ...
            self.photos(
                user_id=user_id, db_client_id=client_id,
                user=user, suitable=suitable, counter=0, user_city=city
                )

    def listen(self):
        """Начало логики бота.
        Тут создаем LongPoll соединение и получаем ответы в реальном времени.
        Первое условие реализовано больше для пользователей,
         которые зашли впервые, у них будет кнопка Start & Начать.
                ...
                if event.to_me:
                    ...
                    if request.lower().strip() in ['start', 'начать']:
                        self.write_msg(
                                event.user_id,
                                '''Здравствуйте,
                                для поиска людей нажмите на кнопку.''',
                                keyboard=start_search.get_keyboard())
